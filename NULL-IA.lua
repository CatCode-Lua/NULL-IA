...
ToggleButton.Visible = true -- Siempre visible
...
local function LoadSettings()
    ...
    local function ApplyThemeColor(themeName)
        ...
        -- Actualizar color de texto en todas las pestañas
        for _, child in pairs(ContentArea:GetDescendants()) do
            if child:IsA("TextLabel") and child.TextColor3 == Themes[Config.Theme].Text then
...
- Sistema de UI completa.
- Soporte para temas (4 colores).
- Chat IA simulado con respuestas.
- Ejecutor de scripts Lua básico integrado.

[v1.1 Actual]
- Sin sistema de Key (acceso directo).
- Funciones de Menú y PvP.
- Configuración de temas dinámica.
- Sistema de toggles funcional.
...
-- ================= MEJORAS: SISTEMA DE NOTIFICACIONES =================
local NotificationService = {}

function NotificationService:SendNotification(title, text, duration)
    game:GetService("StarterGui"):SetCore("SendNotification", {
        Title = title or "NULL-IA",
        Text = text or "Notificación",
        Duration = duration or 5
    })
end

-- ================= MEJORAS: SISTEMA DE TOGGLES REALES =================
local ToggleFunctions = {
    desync = function(state)
        if state then
            print("[DESYNC ACTIVADO] - Modo desincronización activo")
            NotificationService:SendNotification("NULL-IA", "Desync Activado")
        else
            print("[DESYNC DESACTIVADO]")
            NotificationService:SendNotification("NULL-IA", "Desync Desactivado")
        end
    end,
    
    esp_p = function(state)
        if state then
            print("[ESP PLAYERS ACTIVADO] - Visualización de jugadores activa")
            -- Aquí iría el código real para ESP
        else
    ...
    aim = function(state)
        if state then
            print("[AIMBOT ACTIVADO] - Sistema de aim asistido")
            NotificationService:SendNotification("NULL-IA", "Aimbot Activado")
        else
            print("[AIMBOT DESACTIVADO]")
            NotificationService:SendNotification("NULL-IA", "Aimbot Desactivado")
        end
    end
}

-- Actualizar la función CreateToggle para usar funciones reales
local function CreateToggle(parent, text, id, callback)
    local frame = Instance.new("Frame")
    frame.Size = UDim2.new(1, 0, 0, 30)
    frame.BackgroundTransparency = 1
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        if ToggleFunctions[id] then
            ToggleFunctions[id](toggled)
        end
        
        -- Ejecutar callback general si se proporciona
        if callback then 
            callback(toggled) 
        end
    end)
...
-- Mostrar botón flotante y cargar menú por defecto
ToggleButton.Visible = true
MainFrame.Visible = true
LoadMenu()

-- Notificación de inicio
NotificationService:SendNotification("NULL-IA", "Ejecutor cargado correctamente\nVersión 1.1 - Sin Key System", 5)

-- Función para cerrar completamente la UI
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if not gameProcessed then
        if input.KeyCode == Enum.KeyCode.RightControl then
            ScreenGui:Destroy()
            print("NULL-IA cerrado (RightControl)")
            NotificationService:SendNotification("NULL-IA", "Interfaz cerrada", 3)
        elseif input.KeyCode == Enum.KeyCode.F then
            MainFrame.Visible = not MainFrame.Visible
            print("Toggle UI (F key)")
        end
    end
end)

-- ================= ATALLAOS DE TECLADO MEJORADOS =================
local Keybinds = {
    [Enum.KeyCode.Insert] = function()
        MainFrame.Visible = not MainFrame.Visible
        print("Toggle UI (Insert key)")
    end,
    
    [Enum.KeyCode.End] = function()
        ScreenGui:Destroy()
        print("NULL-IA cerrado (End key)")
    end,
    
    [Enum.KeyCode.F9] = function()
        if Config.Toggles["aim"] then
            Config.Toggles["aim"] = not Config.Toggles["aim"]
            ToggleFunctions["aim"](Config.Toggles["aim"])
            print("Toggle rápido Aimbot (F9)")
        end
    end
}
...
print("======================================")
print("NULL-IA EXECUTOR v1.1")
print("Cargado correctamente")
print("Controles:")
print("- Insert: Mostrar/Ocultar interfaz")
print("- F: Alternar UI")
print("- End: Cerrar completamente")
print("- F9: Toggle rápido Aimbot")
print("======================================")

-- Mantener script activo
local heartbeat = RunService.Heartbeat:Connect(function()
    -- Aquí se pueden añadir funciones que se ejecuten cada frame
    if Config.Toggles["desync"] then
        -- Ejemplo: Código para desync se ejecutaría aquí
    end
end)
...
ScreenGui.Destroying:Connect(function()
    ...
    print("NULL-IA: Recursos liberados")
end)